...
public class Iti67RequestUpdateConverter extends Iti65RequestConverter {
  ...
  public SubmitObjectsRequest convertDocumentReferenceToDocumentEntry(@Body DocumentReference documentReference) {
    ...
    Extension repositoryUniqueIdExtension = documentReference
            .getExtensionByUrl(MagConstants.FhirExtensionUrls.POST_REPOSITORY_UNIQUE_ID);
    if (repositoryUniqueIdExtension != null && repositoryUniqueIdExtension.getValue() instanceof Identifier) {
      ...
      entry.setRepositoryUniqueId(identifier.getValue());
    }

    entry.setDocumentAvailability(DocumentAvailability.ONLINE);
    ...
    String version;
    Extension versionExtension = documentReference.getExtensionByUrl(MagConstants.FhirExtensionUrls.POST_DOCUMENT_ENTRY_VERSION);
    if ((versionExtension != null) && (versionExtension.getValue() instanceof Identifier)) {
      Identifier identifier = (Identifier) versionExtension.getValue();
      version = identifier.getValue();
    } else {
      version = "1";
    }
  ...
  private Association createHasMemberAssociationWithOriginalPreviousLabel(String version, SubmissionSet submissionSet, DocumentEntry entry) {
    var assoc = createHasMemberAssociation(entry.getEntryUuid(), submissionSet);
    assoc.setLabel(AssociationLabel.ORIGINAL);
    assoc.setPreviousVersion(version);
