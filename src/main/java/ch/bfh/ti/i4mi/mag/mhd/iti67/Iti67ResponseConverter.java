...
import ch.bfh.ti.i4mi.mag.mhd.iti68.Iti68RequestConverter;
...
public class Iti67ResponseConverter extends BaseQueryResponseConverter {
    ...
    @Override
    public List<Resource> translateToFhir(QueryResponse input, Map<String, Object> parameters) {
        ArrayList<Resource> list = new ArrayList<>();
        if (Status.FAILURE.equals(input.getStatus())) {
            processError(input);
        }
        if (Status.PARTIAL_SUCCESS.equals(input.getStatus())) {
            list.add(processErrorAsOutcome(input));
        }
        ...
        if (input.getDocumentEntries() != null) {
            for (DocumentEntry documentEntry : input.getDocumentEntries()) {
                ...
                String retrieveUrl = config.getUriMagXdsRetrieve() +
                        '?' + Iti68RequestConverter.DOCUMENT_UNIQUE_ID_URL_PARAM + '=' + documentEntry.getUniqueId() +
                        '&' + Iti68RequestConverter.REPOSITORY_UNIQUE_ID_URL_PARAM + '=' + documentEntry.getRepositoryUniqueId();

                if (StringUtils.isNotBlank(documentEntry.getHomeCommunityId())) {
                    documentReference.addExtension(
                            MagConstants.FhirExtensionUrls.HOME_COMMUNITY_ID,
                            new Identifier()
                                    .setValue(documentEntry.getHomeCommunityId())
                                    .setSystem(MagConstants.FhirCodingSystemIds.RFC_3986));

                    retrieveUrl = retrieveUrl + '&' + Iti68RequestConverter.HOME_COMMUNITY_ID_URL_PARAM + '=' + documentEntry.getHomeCommunityId();
                }

                // retrievable location of the document -> content.attachment.url uri
                // [0..1] [1..1
                // has to be defined, for the PoC we define
                // $host:port/camel?repositoryUniqueId=...&uniqueId=...[&homeCommunityId=...]
