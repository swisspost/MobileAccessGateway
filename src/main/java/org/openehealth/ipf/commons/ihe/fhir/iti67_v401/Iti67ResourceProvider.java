...
public class Iti67ResourceProvider extends AbstractPlainProvider {
    ...
    /**
     * PATCH OE Handles DocumentReference Update. This is not an actual part of the ITI-67 specification, but in the
     * context of restful FHIR IHE transaction for the Swiss EPR it makes sense to invoke the ITI-57 transaction with
     * the metatadata flag deletionRequest set https://github.com/i4mi/MobileAccessGateway/issues/71
     *
     * @param id                  resource ID
     * @param httpServletRequest  servlet request
     * @param httpServletResponse servlet response
    ...
    public MethodOutcome update(
            @ResourceParam DocumentReference theReference,
            @IdParam IdType id,
            RequestDetails requestDetails,
            HttpServletRequest httpServletRequest,
            HttpServletResponse httpServletResponse) {
        if (id == null) throw new InvalidRequestException("Must provide ID with UPDATE request");
        // Run down the route
